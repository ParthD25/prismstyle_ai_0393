...
def flutter_additional_ios_build_settings(target)
  ...
  # Fix: Resolve FLUTTER_ROOT from Generated.xcconfig to find artifacts
  generated_config_path = File.expand_path(File.join('..', 'Generated.xcconfig'), __FILE__)
  config = flutter_parse_xcconfig_file(generated_config_path)
  flutter_root = config['FLUTTER_ROOT']
  
  if flutter_root
    artifacts_dir = File.join(flutter_root, 'bin', 'cache', 'artifacts', 'engine')
  else
    # Fallback (incorrect for local copy, but kept for reference)
